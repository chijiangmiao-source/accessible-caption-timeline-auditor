...
- **选中**：时间轴中每个片段可点击、可聚焦（`Tab` 进入，`Enter`/`Space` 选中）；
  问题结论中的双方 id 也是按钮，点击即选中对应片段并把焦点交给时间轴。
  选中后页面同步展示该片段的 id、起止毫秒、时长与完整正文。
- **键盘切换**：`←`/`→` 按稳定排序逐条切换；边界不循环，到达首条/末条时给出
  可感知提示（可见文本 + `role="status"` 实时播报）。
- **定位**：切换选中时保持当前缩放比例，自动平移视口到完整露出目标片段。
- **选择契约**：选择状态仅以片段 id 记录。输入变化后若该 id 仍存在则保留选择；
  目标消失、整批非法或清空输入时撤销详情。
...
src/
  ...
  locate.ts     选中解析、方向键顺序切换、保持缩放的自动平移
  Timeline.tsx  可缩放 SVG 时间轴（滚轮/按钮缩放、拖拽平移、联动高亮、选中与键盘定位）
  App.tsx       页面装配、结论展示与选中详情
